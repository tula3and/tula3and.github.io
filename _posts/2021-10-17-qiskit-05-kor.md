...
qp.binary_var('x')
qp.binary_var('y')
qp.integer_var(name='z', lowerbound=0, upperbound=7)
...
잘 설명되어 있습니다.
해당 페이지에 나와있는 여러 변환기 중에서 `IntegerToBinary`를 이용해보도록 하겠습니다.

```python
from qiskit_optimization.converters import IntegerToBinary

int2bin = IntegerToBinary()
qubo = int2bin.convert(qp)
print(qubo.prettyprint())
```

QUBO 형태로 변환이 완료되었다면 이제 `MinimumEgenOptimizer`를 통해 `qubo`의 최적해를 구할 수 있습니다.
QAOA, VQE, CplexSolver (classical exact method) 등 여러 방법이 있지만, 이번에는 QAOA를 사용해서 해를 구해보겠습니다.
...
지나는 간선의 수(+ 가중치가 있을 경우 각 간선의 가중치 합)가 최대가 되도록 하는 해를 찾는 최적화 문제입니다.
